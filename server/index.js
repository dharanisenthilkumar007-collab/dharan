...
app.post("/api/auth/register", async (req, res, next) => {
  try {
    ...
    const user = await User.create({ name, phone, passwordHash: await bcrypt.hash(password, 12), paymentId, qrPayload: `enerpay://pay/${paymentId}` });
...
app.get("/api/me", auth, async (req, res, next) => { try { res.json({ user: publicUser(await User.findById(req.userId)) }); } catch (e) { next(e); } });
app.get("/api/users/lookup", auth, async (req, res, next) => {
  try {
    const q = (req.query.q || "").trim();
    const user = await User.findOne({ $or: [{ paymentId: q.toLowerCase() }, { phone: q }] });
...
app.post("/api/payments", auth, async (req, res, next) => {
  ...
  try {
    ...
    if (!recipient || !["money", "energy"].includes(kind) || !Number.isFinite(value) || value <= 0) return res.status(400).json({ message: "Enter a valid recipient, payment type, and amount." });
    await session.withTransaction(async () => {
      const sender = await User.findById(req.userId).session(session);
      const receiver = await User.findOne({ $or: [{ paymentId: recipient.toLowerCase() }, { phone: recipient }] }).session(session);
      ...
      const field = kind === "money" ? "balances.moneyInr" : "balances.energyKwh";
      if (sender.balances[kind === "money" ? "moneyInr" : "energyKwh"] < value) throw Object.assign(new Error("Insufficient balance."), { status: 400 });
      await User.updateOne({ _id: sender._id }, { $inc: { [field]: -value } }, { session });
      await User.updateOne({ _id: receiver._id }, { $inc: { [field]: value } }, { session });
      await Transaction.create([{ reference: `EP-${crypto.randomUUID().slice(0, 8).toUpperCase()}`, kind, amount: value, sender: sender._id, receiver: receiver._id, note }], { session });
    });
...
app.get("/api/transactions", auth, async (req, res, next) => {
  try { const transactions = await Transaction.find({ $or: [{ sender: req.userId }, { receiver: req.userId }] }).populate("sender receiver", "name paymentId").sort({ createdAt: -1 }).limit(100); res.json({ transactions }); } catch (e) { next(e); }
});
...
app.get("/api/analytics", auth, async (req, res, next) => {
  try {
    ...
    const start = new Date(); start.setHours(0,0,0,0); if (period === "daily") start.setHours(start.getHours() - 23); if (period === "weekly") start.setDate(start.getDate() - 6); if (period === "monthly") start.setMonth(start.getMonth() - 11);
    const points = await Transaction.aggregate([{ $match: { receiver: new mongoose.Types.ObjectId(req.userId), kind: "money", createdAt: { $gte: start } } }, { $group: { _id: { $dateToString: { format, date: "$createdAt", timezone: "Asia/Kolkata" } }, earnings: { $sum: "$amount" } } }]);
